...
const TEMPLATE_PRESETS: TemplatePresetMeta[] = [
  ...
  { id: 'huggingface', name: 'Hugging Face', category: 'recommended', description: '全球最大开源模型社区官方推理路由' },
  ...
  { id: 'hunyuan', name: '腾讯混元 (Hunyuan)', category: 'domestic', description: '腾讯官方混元通用大模型' },
...
export const ProviderAddModal: React.FC<ProviderAddModalProps> = ({ isOpen, onClose, onSelectTemplate }) => {
  ...
  const filteredPresets = useMemo(() => {
    ...
    const list = TEMPLATE_PRESETS.filter((preset) => {
      if (!q) return true;
    ...
    return list.slice().sort((a, b) => {
      const nameA = getPresetDisplayName(a);
      const nameB = getPresetDisplayName(b);
      return nameA.localeCompare(nameB, 'en', { sensitivity: 'base', numeric: true });
    });
  }, [search, getPresetDisplayName]);
